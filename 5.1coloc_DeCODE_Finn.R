...
for (i in same_exp_metasig) {
  load("M:/pQTL_BC_MR/0BC_data_finngen_R9/finndataR9_BC_as_coloc.rdata")

  setwd('F:/deCODE_pQTL data/')
  allp <- list.files(pattern = '*.gz')
  QTL <- fread(allp[grep(paste0("_",i,'_'),allp)[1]],data.table = F, integer64 = "numeric")
  
  data1 <- QTL %>% dplyr::select("rsids","Chrom","Pos","effectAllele","otherAllele",
  ...
  QTLdata <- QTLdata[QTLdata$SNP %in% sameSNP, ] %>% dplyr::arrange(SNP) %>% na.omit() %>% 
    separate_rows(SNP,sep = ',') %>% dplyr::distinct(SNP,.keep_all = T)
  ...
  setwd("M:/pQTL_BC_MR/7.1coloc_DeCODE_Finn/")
  save(result,file =paste0(i,'_finn_coloc.rdata'))
  ...
  p <- locuscomparer::locuscompare(in_fn1 ='GWAS_coloc.tsv',  in_fn2 ='QTL_coloc.tsv',
                                   title1 = "Breast cancer risk (FinnGen)",title2 = paste0(i,"(pQTL)"),
                                   genome = "hg38",legend_position = 'topleft')
  
  pdf(file =paste0(i,"_Finn_coloc.pdf"),width = 10,height = 8)
  print(p)
  dev.off()
...
setwd("M:/pQTL_BC_MR")
save(all_coloc_results,file = '5.1coloc_DeCODE_Finn.rdata')
fwrite(all_coloc_results,file ='5.1coloc_DeCODE_Finn.csv',row.names = T) 
