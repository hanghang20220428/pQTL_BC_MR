...
load("M:/pQTL_BC_MR/all_pQTL_mrres_BCAC.rdata")
...
ggplot(AllOR,aes(x = b, y = -log10(FDR),color=Effect)) +
  ...
  labs(x = "Beta (effect size)", 
       ...
       title = 'All MR results between proteins and BC risk (BCAC_OncoArray)')+
  ggrepel::geom_label_repel(aes(label = label),size = 3,
                            color="black",box.padding = unit(0.4, "lines"), 
                            segment.color = "black",
                            segment.size = 0.4)
ggsave(filename = '2.1volcano_plot_BCAC.pdf',width = 6.5,height = 4.5)              
